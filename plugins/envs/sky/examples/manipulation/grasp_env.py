...
class GraspEnv:
    def __init__(
        ...
        # Gym-style attributes for compatibility with lerobot
        ...
        # PI 0.6 specific attributes
        self.pi0_image_resolution = (224, 224)  # PI 0.6 default resolution
        self.use_images = True  # Enable image-based observations for PI 0.6
    ...
    def get_pi0_observations(self) -> dict:
        """Get observations in PI 0.5 format with images and state."""
        # Get stereo images for PI 0.5
        stereo_images = self.get_stereo_rgb_images(normalize=True)
        
        # Resize images to PI 0.5 resolution (224x224)
        if self.image_width != 224 or self.image_height != 224:
            stereo_images = torch.nn.functional.interpolate(
                stereo_images, size=(224, 224), mode='bilinear', align_corners=False
            )
        
        # Get state observations
        state_obs, _ = self.get_observations()
        
        # Return PI 0.5 compatible observation dict
        return {
            "observation.images": stereo_images,  # Stereo images
            "observation.state": state_obs,      # State vector
        }
